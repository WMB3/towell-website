...
const TowellAssistant = memo(() => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      Tone: Highly professional, polite, and concise corporate tone. Answer any questions about the history enthusiastically.`;
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] font-text flex items-end gap-4">
      {/* AI Hinting Tooltip */}
      {!isOpen && (
        <div className="hidden md:flex bg-white/10 backdrop-blur-md text-white border border-white/20 px-5 py-3 rounded-full font-text text-sm shadow-xl animate-pulse items-center gap-2 mb-1">
          <Bot size={16} className="text-[var(--secondary)]" /> 
          <span className="font-bold tracking-wide">Ask AI about our 150-year history!</span>
        </div>
      )}
      ...
      <div className="relative">
        <button 
          ...
          className={`w-14 h-14 bg-[var(--primary)] text-white rounded-full flex items-center justify-center shadow-[0_10px_25px_rgba(10,25,47,0.5)] hover:bg-[#15284b] hover:scale-105 transition-all duration-300 border-2 border-[var(--secondary)] z-50 relative`}
        >
          {isOpen ? <X size={24} /> : <MessageSquare size={24} />}
        </button>

        {isOpen && (
          <div className="absolute bottom-20 right-0 w-[90vw] sm:w-96 h-[500px] max-h-[75vh] bg-white border border-slate-200 shadow-2xl rounded-xl flex flex-col overflow-hidden origin-bottom-right animate-in fade-in zoom-in duration-300">
            <div className="bg-[var(--primary)] p-4 flex items-center gap-3 border-b-4 border-[var(--secondary)]">
              <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center p-1 shadow-inner">
                 {/* Using the anniversary badge inside the chat header since it's perfectly round */}
                 <img 
              ...
              <div>
                <h3 className="font-primary font-bold text-white tracking-wide text-sm">Ask Towell</h3>
                <p className="text-[var(--secondary)] text-[10px] uppercase tracking-widest font-semibold mt-0.5">Corporate AI Assistant</p>
              </div>
            </div>

            <div className="flex-1 bg-[var(--surface)] p-4 overflow-y-auto flex flex-col gap-4">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-3.5 rounded-lg text-sm leading-relaxed shadow-sm ${
                    msg.role === 'user' 
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="max-w-[85%] p-3.5 rounded-lg bg-white border border-slate-200 text-slate-400 rounded-tl-none shadow-sm flex items-center gap-2 text-sm">
                    <Loader2 size={16} className="animate-spin text-[var(--secondary)]" /> Analyzing history...
                  </div>
                </div>
              )}
            ...
            <div className="p-4 bg-white border-t border-slate-200">
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className="flex-1 bg-[var(--surface)] border border-slate-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-[var(--secondary)] focus:ring-1 focus:ring-[var(--secondary)] transition-all"
                />
                <button 
                  onClick={handleSend}
                  disabled={isLoading || isPending || !input.trim()}
                  className="bg-[var(--secondary)] text-[var(--primary)] p-2.5 rounded-lg hover:brightness-110 transition-all disabled:opacity-50"
                >
                  <Send size={18} />
                </button>
              </div>
    ...
    <>
      <nav 
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 border-b ${
          isScrolled 
            ? 'bg-white border-slate-200 shadow-md py-3 md:py-4' 
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center cursor-pointer group">
              {/* THE NEW COMBINED LOGO
                  Original: Solid White Background, Blue Text.
                  Dark Nav: invert(1) turns it to Black BG/Orange Text. grayscale(1) turns to Black BG/White Text. mix-blend-screen deletes Black BG!
                  Light Nav: mix-blend-multiply naturally deletes the White BG, leaving the clean dark logo! 
              */}
              <img 
                src="towells-logo-combined.png"
                alt="Towell Group"
                className={`transition-all duration-700 object-contain opacity-95 ${isScrolled ? 'mix-blend-multiply' : 'mix-blend-screen'}`}
                style={{ 
                  height: isScrolled ? '40px' : '55px',
                  filter: isScrolled ? 'grayscale(1) contrast(1.2)' : 'grayscale(1) invert(1) brightness(2)'
                }}
            ...
            <div className="hidden lg:flex items-center space-x-10">
              {NAV_LINKS.map((link) => (
                <a 
                  ...
                  className={`text-[12px] font-accent font-bold tracking-widest uppercase transition-colors relative group ${
              ...
              <button className={`flex items-center gap-2 px-6 py-2.5 transition-all duration-300 font-accent font-bold uppercase tracking-wider text-[12px] border-2 rounded-sm ${
                isScrolled 
...
const Hero = memo(() => {
  return (
    <div className="w-full flex flex-col items-center">
      ...
      {/* Solid dark navy background matching reference image identically */}
      <div className="w-full min-h-[90vh] bg-[var(--primary)] flex flex-col justify-center pt-28 pb-32 lg:pb-40 border-b border-[#142A4A]">
        ...
        <div className="w-full max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col items-start text-left mt-8 lg:mt-16">
            ...
            <Reveal delay={100}>
              {/* Phrase is White BG/Black Text. Invert makes it Black BG/White Text. Screen hides Black BG. */}
              <img 
            ...
            <Reveal delay={300}>
              <h1 className="text-5xl md:text-6xl lg:text-[6rem] font-bold font-primary text-white mb-6 leading-[1.05] tracking-tight">
                Shaping the <br/>
          ...
          {/* Right Column: Replaced stacked logos with the new Combined Logo */}
          <div className="lg:col-span-5 flex flex-col items-center lg:items-end justify-center mt-12 lg:mt-0 pr-0 lg:pr-10">
              <Reveal delay={500}>
                {/* New Combined Logo. White BG -> Invert -> Screen hides BG leaving pure crisp outline */}
                <img 
                  src="towells-logo-combined.png" 
                  alt="Towell Group" 
                  className="w-full max-w-[280px] lg:max-w-[380px] mix-blend-screen object-contain opacity-90 hover:opacity-100 hover:scale-105 transition-all duration-700" 
                  style={{ filter: 'grayscale(1) invert(1) brightness(1.5)' }}
                />
              </Reveal>
          </div>
      ...
      <div className="relative w-full max-w-[1200px] mx-auto px-6 lg:px-12 -mt-12 md:-mt-16 mb-16 z-20">
        <div className="bg-white border border-slate-100 shadow-[0_20px_40px_-10px_rgba(10,25,47,0.2)] flex flex-col lg:flex-row items-center justify-between p-6 md:p-10 rounded-sm">
          ...
          <div className="flex-1 flex justify-center items-center gap-8 md:gap-12">
            {/* White surface logic: mix-blend-multiply universally drops white backgrounds! Perfect for new assets. */}
            <img 
              src="towells-anniversary-badge.png" 
              alt="150 Years"
              className="h-20 md:h-28 lg:h-32 object-contain mix-blend-multiply opacity-95 transition-transform hover:scale-105 duration-500"
            />
            <img 
              src="towells-logo-combined.png" 
              alt="Towell Group"
              className="h-16 md:h-24 lg:h-28 object-contain mix-blend-multiply opacity-95 transition-transform hover:scale-105 duration-500"
          ...
          <div className="lg:col-span-6 flex justify-center mt-10 lg:mt-0">
            <Reveal direction="left" className="w-full max-w-sm md:max-w-md">
              <div className="relative aspect-square w-full bg-[var(--surface)] rounded-full flex items-center justify-center border-[6px] md:border-[8px] border-white p-6 md:p-10 shadow-xl">
                {/* Anniversary Badge on White/Light Surface: Multiply perfectly removes the white background while keeping the rich blue intact */}
                <img 
                  src="towells-anniversary-badge.png" 
                  alt="150+ Years Emblem"
                  className="w-full h-full object-contain mix-blend-multiply transition-transform duration-[2000ms] hover:rotate-[360deg] opacity-95"
                />
              </div>
            </Reveal>
          </div>
          ...
          <div className="lg:col-span-5">
            <div className="flex items-center gap-5 w-fit mb-8">
               <img 
                 src="towells-logo-combined.png" 
                 alt="Towell Group" 
                 className="h-16 md:h-24 w-auto object-contain mix-blend-screen opacity-90" 
                 style={{ filter: 'grayscale(1) invert(1) brightness(1.5)' }}
               />
            </div>
